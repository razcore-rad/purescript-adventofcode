module D06 (part1, part2) where

import Prelude

import Control.Alt ((<|>))
import Data.Array (concat, many, (..))
...
data Instruction
  = On     (Tuple Int Int) (Tuple Int Int)
  | Off    (Tuple Int Int) (Tuple Int Int)
  | Toggle (Tuple Int Int) (Tuple Int Int)
...
tokenParser
  = makeTokenParser
  $ LanguageDef (unGenLanguageDef emptyDef)
  { reservedNames = ["turn", "toggle", "through", "on", "off" ] }

turn :: Parser String Unit
turn = tokenParser.reserved "turn"

toggle :: Parser String Unit
toggle = tokenParser.reserved "toggle"

through :: Parser String Unit
through = tokenParser.reserved "through"

on :: Parser String Unit
on = tokenParser.reserved "on"

off :: Parser String Unit
off = tokenParser.reserved "off"

coordinates :: Parser String (List Int)
coordinates = tokenParser.commaSep tokenParser.decimal

whiteSpace :: Parser String Unit
whiteSpace = tokenParser.whiteSpace

parserCoordinates :: Parser String (Tuple (Tuple Int Int) (Tuple Int Int))
parserCoordinates = do
  ls1 <- pair
  _   <- whiteSpace *> through
  ls2 <- pair
  pure $ Tuple ls1 ls2
  where pair = do
          ls <- coordinates
...
parserOn :: Parser String Instruction
parserOn = do
  Tuple ls1 ls2 <- turn *> on *> parserCoordinates
  pure $ On ls1 ls2

parserOff :: Parser String Instruction
parserOff = do
  Tuple ls1 ls2 <- turn *> off *> parserCoordinates
  pure $ Off ls1 ls2

parserToggle :: Parser String Instruction
parserToggle = do
  Tuple ls1 ls2 <- toggle *> parserCoordinates
  pure $ Toggle ls1 ls2
